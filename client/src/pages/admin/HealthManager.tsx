...
import { MedicalCategoryManager } from '../../components/admin/health/MedicalCategoryManager.tsx';
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <PageHeader
                ...
                description="Manage medical partners, hospitals, and categories."
                action={
                    activeTab === 'partners' && (
            ...
            <div className="flex space-x-1 p-1 bg-slate-900/50 rounded-lg w-fit border border-white/5">
                <Button
                    variant={activeTab === 'partners' ? 'default' : 'ghost'}
                    size="sm"
                    onClick={() => setActiveTab('partners')}
                    className={activeTab === 'partners' ? 'bg-accent text-white shadow-neon-sm' : 'text-slate-400'}
                >
                    <ClipboardList className="w-4 h-4 mr-2" /> Partners
                </Button>
                <Button
                    variant={activeTab === 'categories' ? 'default' : 'ghost'}
                    size="sm"
                    onClick={() => setActiveTab('categories')}
                    className={activeTab === 'categories' ? 'bg-accent text-white shadow-neon-sm' : 'text-slate-400'}
                >
                                        ...
                                        <TableRow key={partner.id} className="group">
                                            ...
                                            <TableCell>
                                                <span className="px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-xs text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-white/5">
                                                    {partner.medicalCategoryName || getTypeName(partner.medicalCategoryBaseType || 0)}
                                                </span>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-slate-900 dark:text-slate-400">
                                                <div className="flex items-center gap-1 text-xs">
